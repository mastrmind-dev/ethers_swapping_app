...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (window.ethSwapData) {
      const ethSwap = new web3.eth.Contract(EthSwap.abi, window.ethSwapData.address)
  ...
  sellTokens = async (tokenAmount) => {
    this.setState({ loading: true })
    await this.state.token.methods.approve(window.ethSwapData.address, tokenAmount).send({ from: this.state.account })
